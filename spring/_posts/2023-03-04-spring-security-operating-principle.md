...
Session을 발급받으면 클라이언트의 쿠키에 `JSESSIONID`라는 키로 SessionID가 저장됨.		
...
- SecurityContextHolder라는 전역 객체 안에 SecurityContext에 인증 객체를 저장하는데, 이 SecurityContextHolder는 ThreadLocal에 저장되기 때문에 각기 다른 쓰레드별로 다른 SecurityContextHolder 인스턴스를 가지고 있어서 사용자 별로 각기 다른 인증 객체를 가질 수 있음.
...
- 시큐리티_Session에 접근해서 Authentication을 꺼내고 거기서 UserDetails 타입 User를 꺼내면 우리가 원하는 유저 객체를 꺼낼 수 있게 되는 것.

## 3) 정리
1. 스프링 시큐리티는 오는 모든 접근 주체에 대해 Authentication를 생성함.
2. Authentication은 SecurityContext에 저장됌.
3. SecurityContext는 SecurityContextHolder가 관리함.
...
<br/>
  * *ThreadLocal: Thread마다 할당된 고유 공간(공유X) 
    1. 다른 Thread로부터 안전함.
    2. get, set, remove api가 있음.
    3. set 한 이후 get할 때 장소의 제약이 없음.  
