...
- 외부 반복자 : index를 이용하는 for문, Iterator를 이용하는 while문
...
> ＊ 병렬(parallel) 처리란? <br>
> 한 가지 작업을 서브 작업으로 나누고, 서브 작업들을 분리된 스레드에서 병렬적으로 처리하는 것. <br>
> 병렬 처리 스트림을 이용하면 런타임시 하나의 작업을 서브 작업으로 자동으로 나누고, 서브 작업의 결과를 자동으로 결합해서 최종 결과물을 생성함. <br>
> 병렬 처리 스트림 선언 예시 : <br>
